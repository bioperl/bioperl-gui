...
subfeatures of a Bio::SeqI compliant object. Basic functionality
for selecting single and multiple map objects is inherent in the object
...
object and can be trapped/bound by the user as they see fit.
Colors and axis-offsets of mapped objects are assigned based on the
"source" tag of the feature object. These are assigned "on the fly"
based on whatever is contained in the BioSeq object provided.

This module requires an updated version of Gregg Helt's original
BioTkPerl modules(version 0.81) which are available from BioPerl.
The original BioTkPerl (version 0.80) is Copyright (c) Gregg Helt, 1995;
Version 0.81 was generated by Mark Wilkinson, PBI-NRC, Oct., 2000.

Zooming routines/events in this module are conceptually based on the Zoom
routines from Genotator (Copyright (c) 1996, The Regents of the
University of California.  Harris, N.L. (1997), Genome Research 7(7):754-762)
...
Each map-widget has several "reliable" tags attached to it. These
tags are FIDxxxx, Source and Strand, Type, and Canvas, where:
...
it is possible to also include the unique index number of that database
entry as a fourth tag on the associated map-widget. To do so, create
your SeqFeature objects with an additional tag "id", where the unique databse
index number is the value of this tag. This index number is then
attached to the widget as a fourth tag with the form:
...
mapped objects back to their original database entries.
Using the selectWithTag or recolorWithTag routines (see below) requires
that you pass the **full tag** as the desired selection (eg. pass "Source
...
=head4 new
	...
	Args :
			...
			optionally the SeqFeature tag you wish to use as the label
...
sub new {
	...
	$self->ZoomFrame($frame->Frame->pack(-side => 'bottom', -fill => 'x')); # the sub-frame to hold the zoom-bar
	
	# assign some additional values required by the Canvas and AnnotMap
	# ***************** TEMPORARY HACK
	$self->MapSeq($SeqObj);  # this should be changed later to make DrawableSequence a real Sequence object
	# *******************************
 	$self->{-range} = [0, ($SeqObj->length)];   	# - This line ensures that the sequences fills the allocated space.
	_prepareSeqFeatures($self); 	              	# - This line sets the "width" (relative to H/V orientation) of the map and is dependant on the number of different feature types
													# within this routine the features are counted and assigned colors and offsets from the map axis.
													# the width of each is thus double (plus strand and minus strand) the largest axis offset of a feature
													# the largest offset value is stored in $self->total_offset
    #**********************************************************************************************************
	#print $SeqObj->length;												
	
    # Create the MapCanvases with correct dimensions
	
    # to make things clearer in the next routine
	# here we figure out which of the two maps is the "wider" and make the default width for both maps equal to this value
	# the "width" of a map depends on how many rows of different features are displayed - i.e. its total offset
	my $map_width = ($self->draft_total_offset > $self->finished_total_offset)?$self->draft_total_offset:$self->finished_total_offset;
	...
	if ($self->{-orientation} eq "horizontal") {
        ...
        # it would be nice to have the option to clear all selections by clicking elsewhere on the canvas, but
        # because of the binding order (and the fact that the canvas itself responds to a click event on a widget)
        # the act of selecting a widget would cause this event to also be triggered...
...
sub _bindMultiSelection {
    ...
    $self->DraftCanvas->Tk::bind("<ButtonPress-1>" => [sub {	shift;$self->dragx1($self->DraftCanvas->canvasx(shift)); $self->dragy1($self->DraftCanvas->canvasy(shift))}, Ev('x'), Ev('y')]);

    $self->DraftCanvas->Tk::bind("<B1-Motion>" => [sub {		shift; my $tx2 = shift; my $ty2 = shift;
                                                                my ($x1, $y1) = ($self->dragx1, $self->dragy1);    # the x1/y1 are stored during the button-press event above
                                                                my $x2 = $self->DraftCanvas->canvasx($tx2);        # convert the global x/y coordinate to the canvas x/y coords
    	       							my $y2 = $self->DraftCanvas->canvasy($ty2);
    								$self->DraftCanvas->delete("withtag", "multi_box");    # delete existing boxes
                                                                $self->FinishedCanvas->delete("withtag", "multi_box"); #  "
                                                                $self->DraftCanvas->createRectangle($x1, $y1, $x2, $y2, -tags => "multi_box");  # create a new one
    															},
    															Ev('x'), Ev('y')]   # as parameters send the global x/y coordinates of the event
    							);

    $self->DraftCanvas->Tk::bind("<ButtonRelease-1>" => [sub {	shift; $self->dragx2($self->DraftCanvas->canvasx(shift)); $self->dragy2($self->DraftCanvas->canvasy(shift));
    															my ($x1, $x2, $y1, $y2) = ($self->dragx1, $self->dragx2, $self->dragy1, $self->dragy2);
    ...
    $self->FinishedCanvas->Tk::bind("<ButtonPress-1>" => [sub {	shift;$self->dragx1($self->DraftCanvas->canvasx(shift)); $self->dragy1($self->DraftCanvas->canvasy(shift))}, Ev('x'), Ev('y')]);

    $self->FinishedCanvas->Tk::bind("<B1-Motion>" => [sub {		shift; my $tx2 = shift; my $ty2 = shift;
                                                                my ($x1, $y1) = ($self->dragx1, $self->dragy1);
                                                                my $x2 = $self->FinishedCanvas->canvasx($tx2);
    															my $y2 = $self->FinishedCanvas->canvasy($ty2);
    															$self->DraftCanvas->delete("withtag", "multi_box");
                                                                $self->FinishedCanvas->delete("withtag", "multi_box");
                                                                $self->FinishedCanvas->createRectangle($x1, $y1, $x2, $y2, -tags => "multi_box");
    															},
    															Ev('x'), Ev('y')]
    							);

    $self->FinishedCanvas->Tk::bind("<ButtonRelease-1>" => [sub {shift; $self->dragx2($self->FinishedCanvas->canvasx(shift)), $self->dragy2($self->FinishedCanvas->canvasy(shift));
    															my ($x1, $x2, $y1, $y2) = ($self->dragx1, $self->dragx2, $self->dragy1, $self->dragy2);
    															if ($x1 > $x2){($x1, $x2) = ($x2, $x1)}
    															if ($y1 > $y2){($y1, $y2) = ($y2, $y1)}
    															if (($x2-$x1 < 10 )||($y2 - $y1 < 10)) {  # set sensitivity
...
sub DoZoom  {	# this is also based conceptually on Nomi Harris' Genotator code
...
sub _prepareSeqFeatures {
	...
	push @Finishedsources, ("gene", _extract_sources(@features));      # this extracts the "source" tag from each feature,
																		# Because of the way BioPerl parses genbank we filter out the "gene" features
																		# in the _extract_sources routine because even exons are called "genes" so the whole terminology
																		# becomes meaningless... that's why we add this feature-type back in here as an exclusively top-level feature
	my @tempsources;     # make variable to temporarily hold the sources for each subfeature
    foreach my $feature(@features){
    	my @subfeatures = $feature->sub_SeqFeature;       # now take the sub-features from each top-level feature
    	my @tempsources = _extract_sources(@subfeatures);   # extract the source names from all subfeatures
    	my $flag;
    	foreach my $tempsource(@tempsources){               # check each one in turn
			$flag = 0;
			foreach my $source(@Finishedsources){           # to see if it exists in the list of Finished sources we already know
    ...
    # ********** SUB FEATURES ******************
	@features = $self->MapSeq->all_SeqFeatures;     # this call returns all features, including top-level "gene" features
	my @Draftsources;
	push @Draftsources, ("EST", _extract_sources(@features), "hand_annotation",);   	# we need an additional class "hand-annotation" to deal with modified exons
																					# so we don't bugger-up primary data
																					# and another class EST to deal with features that are not really part of the sequence we are displaying.
    ...
    $self->DraftSources(\@Draftsources);  # stick it into $self to make it easier to get to from other places
...
sub _drawLabels {	
	...
	#############################################################################
	my $text_width = ($self->{-orientation} eq "horizontal")?0:1;  # for horizontal it is 20 characters long, for vertical it is 1 character wide (essentially vertically written)
	# now draw the labels
	my @sources = @{$sources};
	foreach my $source(@sources){
		my $color = ${$self->current_colors}{$source};
		my $offset = ${$self->current_offsets}{$source};
		if ($self->{-orientation} eq "vertical") { 	# vertical text requires splitting between every character, and splits only occurr before spaces
			$source = join ' ', (split //, $source);    # so we break the string up with spaces between each character.
		}	

		
		if ($self->{-orientation} eq "horizontal") {
			$labels->createText(5, ($self->{-axis_loc}+$offset)-5,  # the -5 is because the AnnotMap function draws the bars using a different centering mechanism relative to simply writing text on the canvas
										-text => $source,
										-fill => $color,
										-width => $text_width,
										#-justify => 'right',
			...
			$labels->createText(($self->{-axis_loc}+$offset)-5, 5,  # the +5 is because the AnnotMap function draws the bars using a different centering mechanism relative to simply writing text on the canvas
										-text => $source,
										-fill => $color,
										-width => $text_width,
										#-justify => 'right',
...
# this routine simply adds a "double_clicked" tag to whatever widget was double-clicked.
# more importantly, it **erases the "double_clicked" tag from any previously double-clicked
# widget on either canvas

}
sub _selectFeature {    # the upshot of this is to identify and box any widgets which are "current" on a particular canvas.
						# A widget with the tag "now_current" comes from another subroutine which has computationally selected
						# this item for whatever reason.  The tag "current" comes from the Tk canvas itself, and is added to a widget
						# when it is clicked by the mouse.  We deal with both of these situations here.
						# SorM holds 'single' or 'multi', and is simply needed to tell the drawSelectionBox routine whether or not
						# to delete any existing selection boxes
						# At the end of this routine, any "current" and "now_current" widget is given the "selected" tag.
    my ($self, $canvas, $map, $SorM) = @_;
    my @exons = $canvas->find('withtag', 'now_current');          # if the widget was selected by a subroutine then it has the "now_current" tag
    if (!@exons){@exons = $canvas->find('withtag', 'current')}    # if it was selected by the mouse it has the "current" tag
                                                                  # only one of these two options will be true under normal circumstances
    my $widgetid = shift @exons;                    	# this is presumed to be a list of only one element - the current widget id
    ...
    my $feature = ${$self->IndexedFeatureList}[$FeatureIndex];   # then extract this Bio::Feature object from the list
    my $start = $feature->start;          # $feature now contains the actual BioSeqFeature object, so we can use the Feature methods...
    my $stop = $feature->end;             # like start, stop, strand, etc.
    my $offset = ${$self->current_offsets}{$source}; # offset from axis always depends on the source tag
    # quickly nab the position that was clicked so that we can zoom around this
    my $current_loc = ($self->{-orientation} eq "horizontal")?($canvas->canvasx($canvas->XEvent->x)):($canvas->canvasy($canvas->XEvent->y));  #/)
...
=head4 mapFeatures
	...
	Title : mapFeatures
	Usage : $FeatureIDs = $MapObj->mapFeatures('draft'|'finished',\@FeatureObj)
	Function : map SeqFeature objects to the 'draft'(white) or 'finished'(blue) maps
...
=head4 getSelectedTags

	Title : getSelectedTags
	Usage : ($FeatureID, $strand, $source, $type, $canvas [, $DB_ID]) = $MapObj->getSelectedTags
	Returns : FeatureID, Source, Strand, Type (i.e. Primary_tag),
	Canvas ('draft' or 'finished'), and Database_Index (if available).
...
=head4 getFeaturesWithTag

 Title : getSelectedFeatures
 Usage : $FeatureHashRef = $MapObj->getFeaturesWithTag(\@taglist)
 Returns : a reference to a hash where the FeatureID is the
 key, and the Bio::SeqFeature Object is the value
 Args : reference to a list of valid tags (see discussion of proper tag format)
	...
	if ($whichmap eq 'draft'){
    	foreach my $tag(@tags) {
    		if ($color eq "default"){
    			...
    			my ($ID, $strand, $source) = _extractTags(\@thesetags);      # specifially to obtain 'source'
    			my $thiscolor = ${$self->current_colors}{$source};                  # get the color associated with 'source'
                $self->DraftCanvas->itemconfigure($tag, -fill => $thiscolor);  # assign that color to the widget
    			
    		} else {
                $self->DraftCanvas->itemconfigure($tag, -fill => $color);  # assign that color to the widget
    	...
    	foreach my $tag(@tags) {
    		if ($color eq "default"){
    			my @thesetags = $self->FinishedCanvas->gettags($tag);       # extract tags
    			my ($ID, $strand, $source) = _extractTags(\@thesetags);      # specifially to obtain 'source'
    			my $thiscolor = ${$self->current_colors}{$source};                  # get the color associated with 'source'
                $self->FinishedCanvas->itemconfigure($tag, -fill => $thiscolor);  # assign that color to the widget
    			
    		} else {
                $self->FinishedCanvas->itemconfigure($tag, -fill => $color);  # assign that color to the widget
    		
    		}	
    	}
	}
	else {print "No known map-type was specified in the call to Recolor - must be either 'draft' or 'finished'"}
 		
}

=pod

=head4 assignCustomColors
...
sub assignCustomColors {
	   ...
	   $cedit = $top->ColorEditor(-title => "chose a new color for the $source features",
	   								-command => sub {my $color = $_[1];
	   												${$self->current_colors}{$source} = $color;
	   												print "new color for $source is " . ${$self->current_colors}{$source} . "\n";
